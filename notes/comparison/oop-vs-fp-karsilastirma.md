...
| 4 | **Decorator** | `OrderService`, `OrderServiceDecorator`, `BasicOrderService`, `GiftWrapDecorator`, `InsuranceDecorator`, `ExpressShippingDecorator` (wrapper zinciri) | `UnaryOperator<Order>`; `OrderEnhancer.giftWrap().andThen(insurance()).andThen(expressShipping())` | 7 → 2 | ~91 → 50 | **FP.** Yeni “dekor” = yeni metod, yeni sınıf değil; zincirleme okunaklı. |
...
| Decorator | `classic/decorator/OrderService.java`, `OrderServiceDecorator.java`, `*Decorator.java`, `BasicOrderService.java` | `modern/decorator/OrderEnhancer.java`, `Order.java` |
