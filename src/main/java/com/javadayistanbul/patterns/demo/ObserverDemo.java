...
public class ObserverDemo {
    ...
    public static void run() {
        ...
        IO.println("  OBSERVER PATTERN");
        IO.println("  Java Feature: Consumer<T> & Method References");
        IO.println("=".repeat(60));
        IO.println();
    ...
    private static void classicApproach() {
        IO.println("--- Klasik Yaklasim (Interface + Concrete Listeners) ---");
        IO.println("  [5 dosya: 1 event + 1 listener interface + 2 concrete + 1 manager]");
        IO.println();
        ...
        IO.println("  Siparis olusturuldu:");
        manager.notify("ORDER_CREATED", new OrderEvent("ORD-001", "ORDER_CREATED", "Yeni siparis alindi"));
        IO.println();
        IO.println("  Siparis kargolandi:");
        manager.notify("ORDER_SHIPPED", new OrderEvent("ORD-001", "ORDER_SHIPPED", "Kargoya verildi"));
    }

    private static void modernApproach() {
        IO.println("--- Modern Yaklasim (Consumer<T> + Lambda) ---");
        IO.println("  [1 dosya: record event + Consumer<OrderEvent> + lambda]");
        IO.println("  Listener interface/class yok! Sadece lambda!");
        IO.println();

        var manager = new com.javadayistanbul.patterns.modern.observer.OrderEventManager();

        manager.subscribe("ORDER_CREATED", event ->
                IO.println("    [Email -> ahmet@email.com] " + event.eventType() + ": " + event.details()));
        manager.subscribe("ORDER_CREATED", event ->
                IO.println("    [SMS -> +90 555 123 4567] " + event.eventType() + ": " + event.details()));
